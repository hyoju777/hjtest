...
public class JunController {
    ...
    // 글 목록
    @GetMapping("/post-list")//클라이 언트가 post-list URL로 GET 요청을 보낼 때 getPostList메서드가 호출되어 포스트의 리스트를 반환한다.
    public String getBlogList(Model model) {// 블로그 목록을 가져와서 모델에 추가하는 메서드입니다.
 /*
 * @param model View로 데이터를 전달하기 위해 사용되는 Model 객체입니다.
 * @return 뷰의 이름을 나타내는 문자열입니다.
                */
